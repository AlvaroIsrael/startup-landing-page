...
const getDataFromLocalStorage = (): AuthState => {
  const token = localStorage.getItem('@StartUpLanding:token');
  const user = localStorage.getItem('@StartUpLanding:user');
...
export const AuthProvider: React.FC = ({ children }: PropsWithChildren<ReactNode>) => {
  ...
  const signIn = useCallback(async ({ email, password }: SignInCredentials) => {
    ...
    localStorage.setItem('@StartUpLanding:token', token);
    localStorage.setItem('@StartUpLanding:user', JSON.stringify(user));
  ...
  const signOut = useCallback(() => {
    localStorage.removeItem('@StartUpLanding:token');
    localStorage.removeItem('@StartUpLanding:user');
